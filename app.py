...
st.markdown("<h1 style='color:#40E0D0;'>📊 PhonePe Pulse - Dashboard</h1>", unsafe_allow_html=True)
...
@st.cache_resource
def get_connection():
    engine = create_engine("mysql+pymysql://root:Vishal%400712@127.0.0.1/phonepay")
    return engine
...
with tab1:
    ...
    with col2:
        ...
        if data_type == "Transactions":
            ...
            with col_districts:
                st.markdown("##### 🏙️ Top 10 Districts")
                if selected_region == "All India":
            ...
            with col_pins:
                ...
                pincode_df = get_top_postal_codes(year, quarter, selected_region)
                pincode_df["Amount"] = pincode_df["Amount"].apply(lambda x: f"₹{x:,.0f}")
                for idx, row in pincode_df.iterrows():
                    st.markdown(f"- **{row['Pincodes']}** : {row['Amount']}")
...
# =============== BUSINESS CASE STUDY TAB ===============
# =============================== TAB 2: BUSINESS CASE STUDY ===============================
...
with tab2:
    ...
    def show_case_study_3():
        ...
        st.markdown("### 🗺️ Insurance Transactions Across States")
        fig_map = px.choropleth(
            map_df,
            geojson=geojson,
            featureidkey='properties.ST_NM',
    ...
    def show_case_study_5():
        ...
        st.markdown("### 🗺️ Registered Users Distribution")
        user_map_query = """
